...
7. **Görsel Kalite ve Boyut İyileştirme (ZORUNLU)**: Dışarıdan indirilen tüm görseller (kapak ve içerik görselleri) gönderiye veya oyuna eklenmeden önce **maksimum 1280px genişliğe (en boy oranını koruyarak)** ölçeklenmeli, renk canlılığı +18%, kontrast +8%, keskinlik +15% ve web-optimize **JPEG kalite 85** ile doğrudan temiz dosya adı üzerine (`<isim>.jpg`) iyileştirilerek kaydedilmelidir. Sabit 2x büyütme yalnızca görselin orijinal genişliği 600px'den küçükse uygulanmalıdır.
